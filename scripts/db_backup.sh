...
BACKUP_DIR=/home/pengabdi/backups/tahfidz
...
RCLONE_REMOTE="${DB_RCLONE_REMOTE:-gdrive-tahfidz:/Tahfidz_App_DB_Backups}"
...
sudo docker exec -t $CONTAINER_NAME pg_dump -U $DB_USER -d $DB_NAME -c | gzip > "$BACKUP_FILE"
...
if [ "$ACTUAL_USER" != "root" ]; then
    sudo -u "$ACTUAL_USER" rclone copy "$BACKUP_DIR" "$RCLONE_REMOTE" -v
    RCLONE_EXIT=$?
else
    rclone copy "$BACKUP_DIR" "$RCLONE_REMOTE" -v
    RCLONE_EXIT=$?
fi
