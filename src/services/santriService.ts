...
export const updateSantri = async (id: number, data: santriRepo.SantriUpdateData) => {
  return santriRepo.updateSantriAndUser(id, data);
};

export const getSantriForOrtu = async (ortuId: number, options: {
  search?: string;
  page?: number;
  limit?: number;
}) => {
  ...
  const { data, totalData } = await santriRepo.findSantriWithPagination(
    { ortuId, search: options.search, page, limit }
  );
...
export const getAllSantriWithPagination = async (page: number, limit: number, search?: string, tahapHafalan?: TahapHafalan, ortuId?: number) => {
  const { data, totalData, totalSantri } = await santriRepo.findSantriWithPagination({ page, limit, search, tahapHafalan, ortuId });
...
export const getRankedSantriWithPagination = async (page: number, limit: number, search?: string, tahapHafalan?: TahapHafalan) => {
  ...
  const { data, totalData } = await santriRepo.getRankedSantriWithPagination(page, limit, search, tahapHafalan);
  const totalPages = Math.ceil(totalData / limit);
